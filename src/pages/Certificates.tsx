...
export default function Certificates() {
  ...
  useEffect(() => {
    const loadFundedAccounts = async () => {
      ...
      const { data, error } = await supabase
        .from("accounts")
        .select("id, challenge_type, account_size, created_at")
        .or("status.eq.funded,and(status.eq.active,challenge_type.eq.instant)")
        .order("created_at", { ascending: false });

      if (error) console.error("Error fetching funded accounts:", error);
      setAccounts(data || []);
      setLoading(false);
    };
  ...
  const downloadCertificate = async (account: FundedAccount) => {
    const certificate = certificateRefs.current[account.id];
    ...
    try {
      ...
      link.download = `primepips-certificate-${account.id.slice(0, 8)}.png`;
      link.click();
  ...
  return (
    <DashboardLayout title="Certificates" subtitle="View certificates for your funded accounts">
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        {!loading && accounts.length > 0 && <div className="grid gap-8 xl:grid-cols-2">{accounts.map((account) => (
          <article key={account.id} className="relative overflow-hidden border-8 border-[#b9a36a] bg-[#e9e4d8] text-[#20272d] shadow-2xl ring-1 ring-[#8b6b28]/60 print:shadow-none">
            <div ref={(element) => { certificateRefs.current[account.id] = element; }} className="relative">
            ...
            <div className="relative px-6 py-9 text-center sm:px-12 sm:py-12">
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#8b6b28]">Certificate of funded status</p>
              ...
              <p className="mx-auto max-w-lg text-sm leading-7 text-[#59636b]">This certificate recognizes that the PrimePips account identified below has reached funded status within the PrimePips platform.</p>
              <div className="mx-auto mt-8 grid max-w-lg grid-cols-2 border-y border-[#d8c58a] text-left">
                <div className="border-r border-[#d8c58a] px-4 py-4"><p className="text-[10px] font-bold uppercase tracking-wider text-[#8b6b28]">Program</p><p className="mt-1 font-serif text-base font-bold">{challengeLabels[account.challenge_type] || account.challenge_type}</p></div>
                <div className="px-4 py-4"><p className="text-[10px] font-bold uppercase tracking-wider text-[#8b6b28]">Account size</p><p className="mt-1 font-serif text-base font-bold">${Number(account.account_size).toLocaleString()}</p></div>
              </div>
              <div className="mt-7 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-[11px] text-[#59636b]"><span className="flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5 text-[#8b6b28]" /> Account record {new Date(account.created_at).toLocaleDateString()}</span><span className="font-mono">Ref #{account.id.slice(0, 8)}</span></div>
            </div>
            ...
            <div className="relative bg-background px-5 py-4 print:hidden"><Button type="button" variant="outline" className="w-full" disabled={downloadingId !== null} onClick={() => downloadCertificate(account)}><Download className="mr-2 h-4 w-4" />{downloadingId === account.id ? "Preparing PNG..." : "Download Certificate"}</Button></div>
          </article>
        ))}</div>}
      </div>
    </DashboardLayout>
