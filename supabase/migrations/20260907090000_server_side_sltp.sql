-- Server-side SL/TP settlement runs independently of trader browsers.
...
DECLARE a accounts%ROWTYPE; p positions%ROWTYPE; x assets%ROWTYPE; r RECORD; existing trade_history%ROWTYPE;
  ...
  v_phase_passed BOOLEAN; target NUMERIC; current_phase INTEGER; caller_role TEXT;
BEGIN
  caller_role := COALESCE(auth.jwt() -> 'app_metadata' ->> 'role', auth.role());
  IF caller_role NOT IN ('admin', 'service_role') AND auth.uid() IS NULL THEN RAISE EXCEPTION 'unauthorized'; END IF;
  ...
  SELECT * INTO a FROM accounts WHERE id = p_account_id
    AND (user_id = auth.uid() OR caller_role IN ('admin', 'service_role')) FOR UPDATE;
...
SELECT cron.unschedule('server-side-sltp')
WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'server-side-sltp');
...
SELECT cron.schedule(
  ...
  $$
    SELECT net.http_post(
      url := (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'supabase_url') || '/functions/v1/server-sltp',
