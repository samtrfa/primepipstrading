-- Existing positions must always be settleable, including after account status changes.
...
BEGIN
  ...
  IF p.id IS NULL OR p.status <> 'open' THEN RAISE EXCEPTION 'position cannot be closed'; END IF;
  IF p_exit_price IS NULL OR p_exit_price <= 0 THEN RAISE EXCEPTION 'invalid exit price'; END IF;
