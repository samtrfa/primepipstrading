...
DECLARE a accounts%ROWTYPE; p positions%ROWTYPE; x assets%ROWTYPE; r RECORD; existing trade_history%ROWTYPE;
  ...
  v_phase_passed BOOLEAN; v_consistency_ok BOOLEAN; target NUMERIC; current_phase INTEGER;
BEGIN
  ...
  v_consistency_ok := a.challenge_type <> 'one_step' OR public.account_consistency_score(a.id) < 30;
  v_phase_passed := CASE
    WHEN a.challenge_type = 'one_step' THEN
      v_consistency_ok AND (COALESCE(a.phase_passed, false) OR (target > 0 AND (new_balance - a.account_size) / a.account_size * 100 >= target AND v_violation IS NULL))
    ELSE
      COALESCE(a.phase_passed, false) OR (target > 0 AND (new_balance - a.account_size) / a.account_size * 100 >= target AND v_violation IS NULL)
  END;
