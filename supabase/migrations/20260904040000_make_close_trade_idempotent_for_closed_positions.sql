-- A stale client may retry after server-side risk enforcement already closed the position.
...
BEGIN
  ...
  END IF;
  IF p.status <> 'open' THEN RAISE EXCEPTION 'position cannot be closed'; END IF;
