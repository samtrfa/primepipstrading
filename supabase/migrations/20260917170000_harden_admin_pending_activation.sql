-- Let an explicit Admin recovery complete even when generic payment reconciliation
-- cannot apply its optional side effects.
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    reconciliation := jsonb_build_object('activated', false, 'error', SQLERRM);
  END;
  ...
  IF account_row.status = 'pending_payment'
     AND (reconciliation ->> 'activated')::boolean IS DISTINCT FROM true THEN
    ...
    WHERE id = p_account_id AND status = 'pending_payment';
  END IF;
