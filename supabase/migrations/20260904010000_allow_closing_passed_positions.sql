-- A passed account may still have positions opened before the phase transition.
...
BEGIN
  ...
  IF NOT FOUND OR p.status <> 'open' OR a.status NOT IN ('active', 'funded', 'failed', 'passed') OR p_exit_price <= 0 THEN RAISE EXCEPTION 'position cannot be closed'; END IF;
